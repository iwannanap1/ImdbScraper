...
class ImdbComponent(QRComponent):
    ...
    def open_website(self):
        try:
            self.browser.open_available_browser(url='https://www.imdb.com/', browser_selection='chrome', headless= False)
    ...
    def extract_movie_data(self, movie_name):
        ...
        try:
            ...
            # Expand search results if the "more results" button is present
            try:
                self.browser.click_element('css=.more-results-ft-chip.ipc-chip.ipc-chip--on-base')
            except:
                pass  # Ignore if button is not present
            ...
            for result in results:
                try:
                    # Extract title
                    title_element = result.find_element("css selector", '.ipc-metadata-list-summary-item__t')
                    title = title_element.text.strip()

                    # Extract year
                    year_elements = result.find_elements("css selector", '.ipc-metadata-list-summary-item__li')
                    year = None
                    ...
                    continue  # Skip result if any error occurs
            ...
            if self.scroll_until_element_found(storyline_xpath):
                try:
                    self.browser.wait_until_element_is_visible(storyline_xpath, timeout=20)
            ...
            if self.browser.is_element_visible(genre_xpath):
                ...
                if genre_elements:
                    genres_string = ", ".join([elem.text.strip() for elem in genre_elements])

            # Extract reviews
            reviews = self.extract_reviews()
            reviews = reviews + ["Not found"] * (5 - len(reviews))  # Fill missing reviews with "Not found"
    ...
    def extract_reviews(self):
        ...
        try:
            reviews_link_xpath = '//div[@data-testid= "reviews-header"]//h3[@class = "ipc-title__text"]'
            ...
            review_elements = self.browser.find_elements('//div[@data-testid = "review-overflow"]//div[@class = "ipc-html-content-inner-div"]')

            if not review_elements:
                return []  # No reviews found

            reviews = [self.browser.get_text(review_element).strip() for review_element in review_elements[:5]]
            return reviews
        except:
